...
if __name__ == '__main__':
    while 1:
        ...
        try:
            im = ps.grab()
            #pixels = get_pixels(Image.open('resourses/3.png'))
            pixels = get_pixels(im)
            minerals, gas, supplay = get_numbers(pixels)
            print(minerals, gas, supplay)
            if int(''.join(map(str,(minerals)))) > 500:
        	    playsound('C:\\test\\sasaiblock\\resourses\\too_many_minerals.mp3')
            if int(''.join(map(str,(gas)))) > 500:
        	    playsound('C:\\test\\sasaiblock\\resourses\\too_many_vespen_gas.mp3')
            occupy_sup, builded_sup = ''.join(map(str,(supplay))).split('/')
            if int(builded_sup) < 200 and float(int(builded_sup) - int(occupy_sup))/int(builded_sup) < 0.15:
    	        playsound('C:\\test\\sasaiblock\\resourses\\sasai_block.mp3')
